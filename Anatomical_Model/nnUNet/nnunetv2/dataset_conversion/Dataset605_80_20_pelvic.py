...
train_ct_names = sorted(glob.glob("/mnt/Enterprise2/shirshak/PENGWIN_TASK/PENGWIN_CT/part*/*.mha"))[:80]
train_mask_names = sorted(glob.glob("/mnt/Enterprise2/shirshak/PENGWIN_TASK/PENGWIN_CT/labels/*.mha"))[:80]

test_ct_names = sorted(glob.glob("/mnt/Enterprise2/shirshak/PENGWIN_TASK/PENGWIN_CT/part*/*.mha"))[80:]
test_mask_names = sorted(glob.glob("/mnt/Enterprise2/shirshak/PENGWIN_TASK/PENGWIN_CT/labels/*.mha"))[80:]
...
if __name__ == '__main__':
    ...
    for index, (test_img_name, test_label_name) in enumerate(tqdm(zip(test_ct_names, test_mask_names)), 1):

        ct_scale_img = sitk.ReadImage(test_img_name)
        ct_scale_img = change_direction(ct_scale_img)
        sitk.WriteImage(ct_scale_img, join(imagests, "PELVIC_" + str(index).zfill(3) + "_0000.nii.gz"))
        ...
        discrete_mask.SetSpacing(ct_scale_mask.GetSpacing())
        discrete_mask.SetOrigin(ct_scale_mask.GetOrigin())
        sitk.WriteImage(discrete_mask, join(labelsts, "PELVIC_" + str(index).zfill(3) + ".nii.gz"))
...
# export nnUNet_raw="/mnt/Enterprise2/shirshak/NewFracSegNet/Anatomical_Model/dataset2/nnUNet_raw"
# export nnUNet_preprocessed="/mnt/Enterprise2/shirshak/NewFracSegNet/Anatomical_Model/dataset2/nnUNet_preprocessed"
# export nnUNet_results="/mnt/Enterprise2/shirshak/NewFracSegNet/Anatomical_Model/dataset2/nnUNet_results"
