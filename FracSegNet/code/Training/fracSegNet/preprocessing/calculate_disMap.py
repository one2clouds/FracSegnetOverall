...
if __name__ == '__main__':
    # =========================== dir selected ================================
    fileName = '/mnt/Enterprise2/shirshak/NewFracSegNet/FracSegNet/dataset/nnUNet_raw/nnUNet_raw_data/Task600_CT_PelvicFrac150/labelsTr/001_LI.nii.gz'
    write_dir = r'/mnt/Enterprise2/shirshak/NewFracSegNet/FracSegNet/distance_map_zzz_tests/'

    # =========================== edge selected ================================
    print('-----------------------------edge selected-------------------------')
    sitk_img = sitk.ReadImage(fileName)
    edgeLabel = calculate_edge(sitk.GetArrayFromImage(sitk_img))
    edgeLabel_img = sitk.GetImageFromArray(edgeLabel)
    edgeLabel_img.SetDirection(sitk_img.GetDirection())
    edgeLabel_img.SetSpacing(sitk_img.GetSpacing())
    edgeLabel_img.SetOrigin(sitk_img.GetOrigin())

    sitk.WriteImage(edgeLabel_img, write_dir+'edge_label.nii.gz')
    print('-----------------------------Finished writing edge-------------------------')

    # =========================== Distance map calculate ================================
    # edgeLabel = 'edge_label_arr_pre.nii.gz'
    distance_map_nii = distance_map(sitk.GetArrayFromImage(sitk_img), edgeLabel)
    distance_map_nii_img = sitk.GetImageFromArray(distance_map_nii)
    distance_map_nii_img.SetDirection(sitk_img.GetDirection())
    distance_map_nii_img.SetSpacing(sitk_img.GetSpacing())
    distance_map_nii_img.SetOrigin(sitk_img.GetOrigin())

    sitk.WriteImage(distance_map_nii_img, write_dir + 'distance_map.nii.gz')